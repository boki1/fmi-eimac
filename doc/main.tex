...
\begin{frame}
	\begin{dfn}\bigskip
		Нека имаме БЧХ код $\mathcal{C}$ над $F_q$ с дължина $q-1$ и конструктивно разстояние $\delta$.
		Тогава за $\mathcal{C}$ казваме, че е \emph{код на Рид-Соломон}.
	\end{dfn}
\end{frame}
...
\begin{frame}[allowframebreaks]
	...
	\framebreak
	\begin{figure}
		\centering \label{fig:code-types}
		\includegraphics[width=.5\textwidth]{code-types.png}
		\caption{,,Йерархия'' на видовете кодове.}
	\end{figure}
\end{frame}

\section{Линейни кодове}
\begin{frame}
...
\begin{frame}
	...
	\]
	\par Търсим $t(x)$, разделяйки $x^3i(x)$ на $g(x)$, т.е. $x^6+x^3$ на $x^3+x+1$. Получаваме частно $q(x)=x^3+x$ и остатък $r(x)=x^2+x$. Тогава $t(x)=-r(x)=x^2+x$, понеже говорим за двоични кодове, т.е. в $GF(2)$.
\end{frame}
\begin{frame}
	\egmarker
	\par Кодовата дума накра съответства на полинома \[
	c(x)=x^6+x^3+t(x)=x^6+x^3+x^2+x
	\]
	\par A самата кодова дума е $1001110$.
\end{frame}

\begin{frame}[allowframebreaks]
	\par За да декодираме циклични кодове, въвеждаме следното понятие.
	\begin{dfn}\bigskip
		Нека имаме цикличен $(n,k)$ код $\mathcal{C}=\langle g \rangle$. Полиномът $h(x)$, определен от \[
			h(x) := \frac{x^n-1}{g(x)}
		\]
		наричаме \emph{проверочен полином} на кода.
	\end{dfn}
	\framebreak
	\par За него е верен следният резултат.
	\begin{thm}\bigskip
		Нека имаме цикличен $(n,k)$ код $\mathcal{C}$. Тогава вектор $c\in\mathbb{F}^n$ e кодова дума \textsc{тстк} за $c(x)$ е изпълненo \[
			h(\omega).c(\omega)=0
		\]
		където $\omega$ e $n$-ти корен на единицата.
	\end{thm}
\end{frame}
\begin{frame}[allowframebreaks]
	\par Знаем, че всяка кодова дума на цикличния код $\mathcal{C}=\langle g \rangle$ се дели без остатък на пораждащия полином. Понеже две думи дават един и същи остатък при деление на $g(x)$ \textsc{тстк} тяхната разлика дава остатък $0$, т.е. е кодова дума, получаваме, че думите, които имат един и същи остатък на $g(x)$ принадлежат на един и същи съседен клас на кода. Въвеждаме понятието синдром (за цикличен код).
	\framebreak
	\begin{dfn}\bigskip
		Нека имаме цикличен код $\mathcal{C}=\langle g \rangle$. Тогава синдром на кодовата дума $a$ (със съответен полином $a(x)$), както и на целия съседен клас $a+\mathcal{C}$ наричаме полинома $s(x)$, определен от \[
			a(x) = q(x).g(x)+s(x)
		\]
...
\end{frame}
\begin{frame}[allowframebreaks]
	\par Разбира се, тъй като цикличните кодове същинско подмножество на линейните кодове, декодирането е напълно възможно и чрез вече разгледания подход с таблица на синдромите.
	\par Директна полза от цикличните кодове обаче е възможността за по-бързо декодиране, а именно т.нар. \textit{декодер на Мегит}.
	\framebreak
	\par При него таблицата на синдромите се съкращава значително, поради следното наблюдение. Прилагайки циклично завъртане на всички думи от един съседен клас на цикличен код $\mathcal{C}$, получаваме отново съседен клас на същия този код $\mathcal{C}$.
	\par Поради тази причина в таблицата участват само синдромите на онези съседни класове, които имат лидер с ненулев стари коефициент пред $x^{n-1}$.
	\framebreak
	\par Алгоритъмът изглежда по следния начин. Отново разполагаме с цикличен $(n,k)$ код $\mathcal{C}=\langle g \rangle$. При преноса на кодова дума $c$ на другия край на канала достига дума $y$, която не е непременно кодова, ами е променена с грешка $e$ - \[
		y = c + e
	\]
	За да декодираме, съставяме съкратена таблица със синдроми, в която участват единствено синдромите на съседните класове, които имат лидер с ненулев коефициент пред $x^{n-1}$. Тогава първо пресмятаме синдрома на $у$ и го сравняваме с наличните синдроми от таблицата. Ако намерим същия, поправяме грешката и сме готови. Ако търсения синдром не е в таблицата, то получената дума $y$ се измества циклично в $\hat{y}$. Тогава пресмятаме синдрома на $\hat{y}$ и търсим из таблицата. Продължаваме така, докато намерим синдром в таблицата.
\end{frame}
\begin{frame}
	\par \textit{Защо са особено интересни цикличните кодове?} Дори без да сме запознати с особеностите на още по-фините БЧХ кодове и кодове на Рид-Соломон, можем да забележим следния факт. Съществува естествено хардуерна асоциация с цикличните кодове, а именно изместващ регистър. Той има две основни ,,операции'' - \textit{изместване} и \textit{обратна връзка}. Първата съвпада еднозначно с основната операция на цикличните кодове - цикличното завъртане; а втората - с операцията събиране в полето $GF(2)$, реализирано чрез \textsc{изключващо или}.
	\par Създаване на хардуерни ускорители или копроцесори с цел кодиране и декодиране на циклични кодове и широко разпространено.
\end{frame}

\section{Нататък}
\begin{frame}[allowframebreaks]
	\par Следващите две стъпки от нашия преглед са към БЧХ кодовете и кодовете на Рид-Соломон.
	\par Основната идея и на двата вида кодове е да изразим цикличния код чрез корените на пораждащия полином $g$. Тогава използваме факта, че можем да фиксираме полином от $n$-та степен с кои да е $n+1$ точки от него.
	\framebreak
	\begin{dfn}\bigskip
		...
		примитивен $n$-ти корен на 1 над
		полето $F_q$. \emph{БЧХ код} над $F_q$ с дължина $n$ и конструктивно разстояние $\delta$
		е цикличен код с дължина $n$ и пораждащ полином \[
		g(x) = [M_{\alpha^b}(x), M_{\alpha^{b + 1}}(x), \dots, M_{\alpha^{b + \delta
				- 2}}(x)],
		\]
		където $M_{\alpha^s}$ е минималния полином на елемента $\alpha ^ s$ над $F_q$.
	\end{dfn}
	\framebreak
	\begin{dfn}\bigskip
		Нека имаме БЧХ код $\mathcal{C}$ над $F_q$ с дължина $q-1$ и конструктивно разстояние $\delta$.
		Тогава за $\mathcal{C}$ казваме, че е \emph{код на Рид-Соломон}.
	\end{dfn}
	\par Основното негово предимство е ефикасния декодер - т.нар. декодер на Берлекамп-Мейси, който е приложим. Също така от основната теорема за БЧХ кодове, получаваме конструктивен алгоритъм за създаване на БЧХ кодове, които поправят до $t$ на брой грешки. 
\end{frame}
